...
int main(int argc, char *argv[]) {
    if (argc < 4) {
        fprintf(
           stderr,
           "[fast_cksum_cat Error] Incorrect number of arguments.\nUsage: fast_cksum_cat -c CHECKSUM_FILE FILENAME [FILENAME ...]\n"
        );
        exit(1);
    }
    ...
    if (ret != 0 || buffer == NULL) {
        fprintf(
           stderr,
           "[fast_cksum_cat Error] Failed to allocate %" PRIu64 " bytes\n",
           BUFSIZE
        );
        exit(1);
    }
    ...
    if (cfp == NULL) {
        if (errno)
            ...
            fprintf(
               stderr,
               "[fast_cksum_cat Error] File %s not found or cannot be opened.\n",
               checksum_fn
            );
        exit(1);
    }
    ...
    while (getline(&line, &linen, cfp) != -1) {
        ...
        if (count != 3) {
            fprintf(
               stderr,
               "[fast_cksum_cat Error] Did not understand line \"%s\" in checksum file %s\n",
               line,
               checksum_fn
            );
            exit(1);
        ...
        if (known_sizes.count(_fn) || known_crcs.count(_fn)) {
            fprintf(
               stderr,
               "[fast_cksum_cat Error] Duplicate entry \"%s\" in checksum file %s!\n",
               _fn,
               checksum_fn
            );
            exit(1);
    ...
    for (int i = 0; i < ninputs; i++) {
        ...
        if (!known_crcs.count(fn)) {
            fprintf(
               stderr,
               "[fast_cksum_cat Error] File \"%s\" not found in checksum file %s\n",
               fn,
               checksum_fn
            );
            exit(1);
    ...
    for (int i = 0; i < ninputs; i++) {
        ...
        if (totalsize != known_sizes[fn]) {
            fprintf(
               stderr,
               "[fast_cksum_cat Error] Read size %zu from file %s; expected %zu based on checksum file %s\n",
               totalsize,
               fn,
               known_sizes[fn],
               checksum_fn
        ...
        if (crc != known_crcs[fn]) {
            fprintf(
               stderr,
               "[fast_cksum_cat Error] Computed checksum %u from file %s; expected %u based on checksum file %s\n",
               crc,
               fn,
               known_crcs[fn],
               checksum_fn
