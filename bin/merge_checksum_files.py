...
def merge(checksum_files, delete=False):
    ...
    for i,line in enumerate(lines[:-1]):
        if line[-1] == lines[i+1][-1]:
            print(f'[merge_checksum_files.py] Error: duplicate filename {line[-1]} found', file=sys.stderr)
...
if __name__ == '__main__':
    ...
    parser.add_argument('checksum_files', nargs='+', help='One or more checksum files to merge', metavar='FILE')
